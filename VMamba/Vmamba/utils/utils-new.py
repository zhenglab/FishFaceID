...
def load_checkpoint_ema(config, model, optimizer, lr_scheduler, loss_scaler, logger, model_ema: ModelEma=None):
    ...
    if 'model' in checkpoint:
        # 调整权重的形状
        if checkpoint['model']['classifier.head.weight'].shape != model.classifier.head.weight.shape:
            logger.info("Adjusting classifier.head.weight to match current model shape")
            checkpoint['model']['classifier.head.weight'] = torch.nn.init.xavier_uniform_(
                torch.empty_like(model.classifier.head.weight))
        if 'classifier.head.bias' in checkpoint['model'] and checkpoint['model']['classifier.head.bias'].shape != model.classifier.head.bias.shape:
            logger.info("Adjusting classifier.head.bias to match current model shape")
            checkpoint['model']['classifier.head.bias'] = torch.nn.Parameter(
                torch.randn_like(model.classifier.head.bias))
