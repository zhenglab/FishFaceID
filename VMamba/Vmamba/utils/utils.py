...
def load_pretrained_ema(config, model, logger, model_ema: ModelEma=None):
    ...
    print(checkpoint['model']['classifier.head.weight'].shape[0])
    print('#########################################24252q52525')
    
    if checkpoint['model']['classifier.head.weight'].shape[0] == 1000:
        checkpoint['model']['classifier.head.weight'] = torch.nn.Parameter(torch.nn.init.xavier_uniform(torch.empty(config.MODEL.NUM_CLASSES, 768)))
        checkpoint['model']['classifier.head.bias'] = torch.nn.Parameter(torch.randn(config.MODEL.NUM_CLASSES))
        msg = model.load_state_dict(checkpoint['model'], strict=False)
