...
_C.TEST.CROP_TYPE = "center" # 这个type暂时没用[@suen]
_C.TEST.FIVE_CROP = True  # 用来标记是否开启five-crop，在build.py中可以作为判断条件[@suen]
_C.TEST.EVAL_SAVE = False
...
def update_config(config, args):
    ...
    if _check_args('subset'):
        config.DATA.SUBSET = args.subset
    if _check_args('split'):
        config.DATA.SPLIT = args.split
    if _check_args('channel'):
        config.DATA.CHANNEL = args.channel
    if _check_args('test_five_crop'):
        config.TEST.FIVE_CROP = args.test_five_crop
