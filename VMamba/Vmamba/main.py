...
def parse_option():
    ...
    parser.add_argument('--split', type=str, help='path to dataset')
    parser.add_argument('--subset', type=str, help='path to dataset')
...
def main(config, args):
    ...
    if config.MODEL.PRETRAINED and (not config.MODEL.RESUME):
        ...
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        print("@@@@@@@@@@")
        if config.TEST.EVAL_SAVE:
            # Get the parent directory of the resume path
            exp_dir = os.path.dirname(config.MODEL.PRETRAINED)
            print("egsege$$$$$$$$$")
            print(exp_dir)
            ...
            if config.TEST.FIVE_CROP:
                output_file = os.path.join(os.path.dirname(exp_dir), f"evaluation_results_log-five.txt")
            else:
                output_file = os.path.join(os.path.dirname(exp_dir), f"evaluation_results_log-center.txt")
            ...
            with open(output_file, "a") as f:
                ...
                f.write(f"Accuracy@1: {acc1}\n")
                f.write(f"Accuracy@5: {acc5}\n")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # 保存最佳模型
        if dist.get_rank() == 0 and acc1 > best_accuracy:
            
            best_accuracy = acc1
            best_epoch = f"best"  # 生成最佳模型保存路径
            save_checkpoint_ema(config, best_epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema)
            logger.info(f"New best model saved at epoch {epoch} with accuracy {best_accuracy:.4f}%")
