...
from augment_exp3 import new_data_aug_generator
...
def main(args):
    ...
    if args.finetune:
        ...
        # interpolate position embedding if necessary
        if 'pos_embed' in filtered_checkpoint_model:
            pos_embed_checkpoint = filtered_checkpoint_model['pos_embed']
            ...
            filtered_checkpoint_model['pos_embed'] = new_pos_embed

        # 加载过滤后的检查点
        model.load_state_dict(filtered_checkpoint_model, strict=False)
    ...
    if args.eval:
        ...
        print(f"Accuracy5 of the network on the {len(dataset_val)} test images: {test_stats['acc5']:.1f}%")
