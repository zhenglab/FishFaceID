...
def evaluate(data_loader, model, device, amp_autocast, args):
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        if args.eval:
            preds = torch.cat((preds, output), dim=0) if preds != None else output
            labels = torch.cat((labels, target), dim=0) if labels != None else target
    ...
    if args.eval:
        np.save(os.path.join(args.output_dir, "preds.npy"), preds.cpu().numpy())
        np.save(os.path.join(args.output_dir, "labels.npy"), labels.cpu().numpy())
